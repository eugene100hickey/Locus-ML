...
star_pairs <- read_csv("data/train-test-3556.csv") |>
  arrange(cor) |>
...
plot_target_good <- spectrum_plot(index = 2600,
                                  is_match = F,
                                  label_height = 0.7,
                                  mag_label_height = 0.001) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank())
plot_match_good <- spectrum_plot(index = 2600,
                                 is_match = T,
                                 label_height = 0.8,
                                 mag_label_height = 0.03)

(plot_target_good / plot_match_good)  +
  plot_annotation(tag_levels = list(c('A', 'B')),
                  tag_prefix = "(",
                  tag_suffix = ")")

plot_target_mid <- spectrum_plot(index = 1075,
                                 is_match = F,
                                 label_height = 0.6,
                                 mag_label_height = 0.01) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank())
plot_match_mid <- spectrum_plot(index = 1075,
                                is_match = T,
                                label_height = 0.5,
                                mag_label_height = 0.01)

(plot_target_mid / plot_match_mid)  +
  plot_annotation(tag_levels = list(c('C', 'D')),
                  tag_prefix = "(",
                  tag_suffix = ")")


plot_target_bad <- spectrum_plot(index = 200,
                                 is_match = F,
                                 label_height = 0.5,
                                 mag_label_height = 0.01) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank())
plot_match_bad <- spectrum_plot(index = 200,
                                is_match = T,
                                label_height = 0.85,
                                mag_label_height = 0.03)

(plot_target_bad / plot_match_bad)  +
